    ...
    try {
          ...
          return new Response(null, {
            ...
            headers: {
              ...
              "Access-Control-Allow-Headers":
                "Content-Type, X-Username, Authorization",
            },
          });
        }
      ...
      if (pathname === "/api/auth/social-config") {
        ...
        return new Response(JSON.stringify({
          naver: naverClientId ? `https://nid.naver.com/oauth2.0/authorize?response_type=code&client_id=${naverClientId}&redirect_uri=${encodeURIComponent(naverRedirect)}&state=${naverState}` : null,
          kakao: kakaoClientId ? `https://kauth.kakao.com/oauth/authorize?response_type=code&client_id=${kakaoClientId}&redirect_uri=${encodeURIComponent(kakaoRedirect)}` : null,
      ...
      const publicPostCommentsMatch = pathname.match(
        /^\/api\/post\/(\d+)\/comments$/,
      );
      if (publicPostCommentsMatch && request.method === "GET") {
        return postComments.onRequestGet({
          env,
          params: { id: publicPostCommentsMatch[1] },
        });
      }
      ...
      const resolveUserIdFallback = async () => {
        const fromHeader = Number(
          getHeader("X-User-Id") || getHeader("X-UserId"),
        );
      ...
      if (!userId) {
        ...
        try {
          ...
          console.log(
            "⚠️ Failed to decode username: - index.js:149",
            originalUsername,
            e,
          );
          // 디코딩 실패 시 원본 사용
        ...
        userId = userRow.user_id;
        console.log(
          "✅ User authenticated: - index.js:168",
          username,
          "userId:",
          userId,
        );
      ...
      if (
        ...
        pathname.startsWith("/api/user/points/")
      ) {
        if (request.method === "GET") {
      ...
      if (
        pathname === "/api/user/items" ||
        pathname.startsWith("/api/user/items/")
      ) {
        if (request.method === "GET") {
      ...
      if (
        ...
        pathname.startsWith("/api/user/stats")
      ) {
        if (request.method === "GET") {
      ...
      if (
        pathname === "/api/shop/purchase" ||
        pathname.startsWith("/api/shop/purchase/")
      ) {
      ...
      const postCommentsMatch = pathname.match(
        /^\/api\/post\/(\d+)\/comments$/,
      );
      if (postCommentsMatch && request.method === "POST") {
        return postComments.onRequestPost({
          request,
          env,
          params: { id: postCommentsMatch[1] },
          userId,
        });
      }
      ...
      if (commentMatch) {
        ...
        if (action === "like" && request.method === "POST") {
          return commentLike.onRequestPost({
            env,
            params: { id: cid },
            userId,
          });
        ...
        if (!action && request.method === "PATCH") {
          return commentById.onRequestPatch({
            request,
            env,
            params: { id: cid },
            userId,
        ...
        if (!action && request.method === "DELETE") {
          return commentById.onRequestDelete({
            env,
            params: { id: cid },
            userId,
          });
      ...
      if (
        ...
        return postById.default(request, {
          env,
          params: { id: authPostMatch[1] },
          userId,
        });
      }
      ...
      if (postLikeMatch && request.method === "POST") {
        return postLike.onRequestPost({
          env,
          params: { id: postLikeMatch[1] },
          userId,
        });
      }

      if (
        pathname === "/api/user/resolve" ||
        pathname.startsWith("/api/user/resolve/")
      ) {
        ...
        switch (action) {
          ...
          case "minigame":
            ...
            if (request.method === "PATCH") {
              return miniGame.onRequestPatch({
                request,
                env,
                params: { id },
                userId,
