...
document.addEventListener("DOMContentLoaded", () => {
      ...
      for (const m of mutations) {
        for (const node of m.addedNodes) {
          ...
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            const nested =
              el && el.querySelector && el.querySelector("#new-challenge-user");
  ...
  const alertOverlay = alertModal
    ? alertModal.querySelector(".popup-overlay")
    : null;
  ...
  // 로그인 화면 내의 회원가입 버튼
  const loginSignupBtn = loginView
    ? loginView.querySelector(".signup-btn")
    : null;
  ...
  // 챌린지 퀵링크 버튼 클릭 시 (추가)
  const challengeQuickBtn = document.querySelector(
    ".challenge-quicklink button",
  );
  ...
  const memberList = detailView
    ? detailView.querySelector(".member-list")
    : null;
  ...
  const detailMain = detailView
    ? detailView.querySelector(".detail-main")
    : null;
  ...
  function bindDetailActions() {
    ...
    if (complete) {
      complete.addEventListener("click", () => {
        document
          .getElementById("challenge-over-modal")
  ...
  if (detailButtons.length > 0 && detailView) {
    detailButtons.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const parentModal =
          btn.closest(".popup-modal") || btn.closest(".modal");
        if (parentModal) {
          ...
          if (challengeModal && !challengeModal.classList.contains("hidden")) {
            ...
            ongoingModal &&
            !ongoingModal.classList.contains("hidden")
          ) {
  ...
  // 새 챌린지 생성 모달 열기/닫기
  // ==========================================
  const createChallengeBtns = document.querySelectorAll(
    ".create-challenge-btn",
  );
  const createChallengeModal = document.getElementById(
    "create-challenge-modal",
  );
  const createModalOverlay = createChallengeModal
    ? createChallengeModal.querySelector(".popup-overlay")
    : null;
  ...
  const editModalOverlay = editChallengeModal
    ? editChallengeModal.querySelector(".popup-overlay")
    : null;
  const updateChallengeBtn = editChallengeModal
    ? editChallengeModal.querySelector(".update-challenge-btn")
    : null;
  ...
  function openEditModal(card, challengeData = null) {
    ...
    if (challengeData) {
      ...
      if (editCategoryInput)
        editCategoryInput.value = challengeData.category || "";
    } else {
      ...
      infos.forEach((p) => {
        if (p.textContent.includes("목표 -"))
          goal = p.textContent.split("목표 -")[1].trim();
      });
  ...
  if (updateChallengeBtn) {
    updateChallengeBtn.addEventListener("click", () => {
      ...
      if (currentEditingChallenge) {
        ...
        updateCardDOM(
          currentEditingCard,
          currentEditingChallenge.name,
          currentEditingChallenge.duration,
          currentEditingChallenge.goal,
          currentEditingChallenge.createdAt,
          newCategory,
        );
      } else {
        updateCardDOM(
  ...
  function updateCardDOM(card, name, duration, goal, startDate, category) {
    ...
    const infos = card.querySelectorAll(".card-info p");
    infos.forEach((p) => {
      if (p.textContent.includes("기간 -"))
        p.textContent = `기간 - ${dateString}`;
      if (p.textContent.includes("목표 -")) p.textContent = `목표 - ${goal}`;
    });
  }

  const initialEditBtns = document.querySelectorAll(
    ".challenge-card .edit-btn",
  );
  ...
  let createdChallenges =
    JSON.parse(localStorage.getItem("createdChallenges")) || [];
  ...
  function saveChallenges() {
    localStorage.setItem(
      "createdChallenges",
      JSON.stringify(createdChallenges),
    );
  }
  ...
  function renderChallenge(challengeData) {
    const { name, duration, goal, userName, createdAt, category } =
      challengeData;
    ...
    const ongoingGrid = ongoingModal
      ? ongoingModal.querySelector(".challenge-grid")
      : null;
    if (ongoingGrid) {
  ...
  if (allCodeBtn) {
    allCodeBtn.addEventListener("click", () =>
      joinChallengeByCode("#all-challenge-code-input"),
    );
  }
  if (allCodeInput) {
  ...
  const ongoingCodeInput = document.getElementById(
    "ongoing-challenge-code-input",
  );

  if (ongoingCodeBtn) {
    ongoingCodeBtn.addEventListener("click", () =>
      joinChallengeByCode("#ongoing-challenge-code-input"),
    );
  }
  if (ongoingCodeInput) {
  ...
  const giveUpCancelBtn = giveUpModal
    ? giveUpModal.querySelector(".cancel")
    : null;
  const giveUpConfirmBtn = giveUpModal
    ? giveUpModal.querySelector(".giveup")
    : null;
  const giveUpOverlay = giveUpModal
    ? giveUpModal.querySelector(".popup-overlay")
    : null;

  // 2단계 최종 포기 확인 모달 (명언과 함께 표시)
  const finalGiveUpModal = document.getElementById("final-give-up-modal");
  const finalGiveUpCancelBtn = finalGiveUpModal
    ? finalGiveUpModal.querySelector(".cancel")
    : null;
  const finalGiveUpConfirmBtn = finalGiveUpModal
    ? finalGiveUpModal.querySelector(".real-giveup")
    : null;
  const finalGiveUpOverlay = finalGiveUpModal
    ? finalGiveUpModal.querySelector(".popup-overlay")
    : null;
  ...
  if (finalGiveUpConfirmBtn) {
    finalGiveUpConfirmBtn.addEventListener("click", () => {
      ...
      if (currentDetailChallengeData) {
        createdChallenges = createdChallenges.filter(
          (c) => c !== currentDetailChallengeData,
        );
        saveChallenges();
      } else if (cardName) {
  ...
  if (completeBtn && challengeOverModal) {
    completeBtn.addEventListener("click", () => {
      ...
      const rankingView = document.getElementById(
        "challenge-over-ranking-view",
      );

      // Check if category is 'study'
      const isStudy =
        currentDetailChallengeData &&
        currentDetailChallengeData.category === "study";
    ...
    const confirmScoreBtn =
      challengeOverModal.querySelector(".confirm-score-btn");
  ...
  let checkedCardIndices =
    JSON.parse(localStorage.getItem(KEY_CHECKED_INDICES)) || [];
  ...
  attCards.forEach((card, index) => {
    card.addEventListener("click", () => {
      ...
      localStorage.setItem(
        KEY_CHECKED_INDICES,
        JSON.stringify(checkedCardIndices),
      );
      ...
      submitBtn.addEventListener("click", () => {
        ...
        if (isEditMode && currentOpenPost) {
          ...
          const activeMenu = document.querySelector(
            ".community-sidebar .menu-item.active",
          );
  ...
  const defaultPostsData = {
    "menu-popular": [
      ...
      },
      {
        ...
        title: "기말고사 계획 도와주세요",
        content:
          "전교 1등이 기말고사 계획 도와주세요!\n\n어제는 문학 위주로 공부했고, 오늘 아침엔 수학 문제 조금 풀었어요.\n이 흐름 참고해서 오늘 공부 계획 추천 부탁드립니다!",
        date: "2025.12.29. 12:15",
        views: 256,
        comments: [
          {
            user: "피타고라스",
            content: "수학만 집중적으로 하세요",
            date: "2025.12.29. 12:15",
            like: 2,
          },
          {
    ...
    "menu-tips": [
      ...
      },
      {
        ...
        like: 10,
        comment: 15,
        title: "테라리아 아이템 파밍 쉽게 하는법",
        content: "팜 만들어서 잠수 태워 놓으세용",
        date: "2025.12.29. 12:15",
        views: 256,
        comments: [],
      },
      {
        ...
        comment: 9,
        title: "경쟁전 마스터 찍는 법",
        content: "픽을 잘하면 됨 픽 못하는 벌레면 걍 접으셈 ㅉ",
        date: "2025.12.29. 12:15",
        views: 256,
        comments: [],
      },
  ...
  if (storedPosts) {
    try {
      ...
      if (!postsData["menu-popular"])
        postsData["menu-popular"] = defaultPostsData["menu-popular"];
      if (!postsData["menu-tips"])
        postsData["menu-tips"] = defaultPostsData["menu-tips"];
      if (!postsData["menu-data"])
        postsData["menu-data"] = defaultPostsData["menu-data"];
      if (!postsData["menu-mypost"])
        postsData["menu-mypost"] = defaultPostsData["menu-mypost"];
    } catch (e) {
  ...
  function renderFeed(menuId) {
    ...
    posts.forEach((post, index) => {
      const isHighlighted =
        (menuId === "menu-popular" || menuId === "menu-tips") && index === 0;
      const borderStyle = isHighlighted
        ? 'style="border: 3px solid #90D1CA;"'
        : "";
  ...
  const closeDetail = postDetailView
    ? postDetailView.querySelector(".close-detail-text-btn")
    : null;
  ...
  const pdTitle = postDetailView
    ? postDetailView.querySelector(".pd-title")
    : null;
  const pdUsername = postDetailView
    ? postDetailView.querySelector(".pd-username")
    : null;
  const pdDateView = postDetailView
    ? postDetailView.querySelector(".pd-date-view")
    : null;
  const pdContent = postDetailView
    ? postDetailView.querySelector(".pd-content")
    : null;
  const pdLike = postDetailView
    ? postDetailView.querySelector(".pd-like")
    : null;
  const pdComment = postDetailView
    ? postDetailView.querySelector(".pd-comment")
    : null;
  const commentsList = postDetailView
    ? postDetailView.querySelector(".comments-list")
    : null;

  const commentInput = postDetailView
    ? postDetailView.querySelector(".comment-input")
    : null;
  const commentSubmit = postDetailView
    ? postDetailView.querySelector(".comment-submit-btn")
    : null;

  // Buttons in Modal
  const editBtn = postDetailView
    ? postDetailView.querySelector(".pd-btn.edit")
    : null;
  const deleteBtn = postDetailView
    ? postDetailView.querySelector(".pd-btn.delete")
    : null;
  ...
  if (commentsList) {
    commentsList.addEventListener("click", (e) => {
      ...
      if (target.dataset.action === "menu") {
        ...
        // Close all others
        document
          .querySelectorAll(".comment-dropdown")
      ...
      // Handle Delete
        ...
        if (comments && comments[idx]) {
          ...
          showCustomConfirm("댓글을 삭제하시겠습니까?", () => {
            ...
            currentOpenPost.comment = Math.max(
              0,
              (currentOpenPost.comment || 0) - 1,
            );
