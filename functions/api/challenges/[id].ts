  ...
  try {
    ...
    if (!challenge)
      return Response.json(
        { success: false, message: "챌린지 없음" },
        { status: 404 },
      );

    let member = null;
    if (typeof userId === "number") {
      member = await env.D1_DB.prepare(
    ...
    if (request.method === "GET") {
      ...
      try {
        ...
        if (hasStatus) {
          members = await env.D1_DB.prepare(
            ...
            .all();
          console.log(
            `[Challenge Detail] Loaded ${members.results?.length || 0} members with status for challenge ${id}`,
          );
        } else {
          console.warn(
            `[Challenge Detail] status column missing for challenge ${id}`,
          );
          members = await env.D1_DB.prepare(
      ...
      const memberCount = Array.isArray(members?.results)
        ? members.results.length
        : 0;
      const maxMembers = Number((challenge as any)?.max_members || 0);
      const startedFlag = await env.D1_DB.prepare(
    ...
    if (request.method === "DELETE") {
      ...
      if (!isStartedNow) {
        ...
        let betPoints = hasBetCol
          ? Number((challenge as any).bet_points || 0)
          : 0;
        if (!hasBetCol) {
          try {
            await env.D1_DB.prepare(
              `CREATE TABLE IF NOT EXISTS challenge_bets (challenge_id INTEGER PRIMARY KEY, bet_points INTEGER NOT NULL)`,
            ).run();
        ...
        if (betPoints > 0) {
          const allMembers = await env.D1_DB.prepare(
            "SELECT user_id FROM challenge_members WHERE challenge_id = ?",
          )
            .bind(id)
            .all();
          ...
          const pointLogCols = Array.isArray(pointLogInfo?.results)
            ? pointLogInfo.results
            : [];
          const pointLogCol = pointLogCols.some((c: any) => c.name === "point")
            ? "point"
            : pointLogCols.some((c: any) => c.name === "points")
              ? "points"
              : null;
          ...
          for (const row of allMembers.results || []) {
            ...
            const wasPointLog = pointLogCol
              ? await env.D1_DB.prepare(
                  `SELECT 1 FROM point_logs WHERE user_id = ? AND reason LIKE ? AND ${pointLogCol} < 0 LIMIT 1`,
                )
            ...
            if (wasPointLog && pointLogCol) {
              await env.D1_DB.prepare(
                ...
                .bind(betPoints, memberId)
                .run();
              await env.D1_DB.prepare(
                `INSERT INTO point_logs (user_id, ${pointLogCol}, reason, created_at) VALUES (?, ?, ?, ?)`,
              )
                .bind(
                  memberId,
                  betPoints,
                  `challenge_bet:${id}:refund`,
