...
export default async function handler(
  ...
  if (request.method === "GET") {
    try {
      ...
      if (code) {
        ...
        if (!row) {
          return Response.json(
            {
              success: false,
              message: "코드에 해당하는 챌린지를 찾을 수 없습니다.",
            },
            { status: 404 },
          );
        }
      ...
      if (countOnly && includeCompleted) {
        ...
        const completedCount = await env.D1_DB.prepare(
      ...
      const rawChallenges = Array.isArray(userChallenges?.results)
        ? userChallenges.results
        : [];
      ...
      if (hasIsStarted) {
        for (const row of rawChallenges) {
          ...
          const computedStarted =
            Number((row as any)?.is_started || 0) === 1 ||
        ...
        for (const row of rawChallenges) {
          ...
          const flag = await env.D1_DB.prepare(
  ...
  if (!["DAILY", "STUDY", "EXERCISE"].includes(category)) {
    ...
    return Response.json(
      {
        success: false,
        message: `카테고리는 DAILY, STUDY, EXERCISE 중 하나여야 합니다. (입력값: ${category})`,
      },
      { status: 400 },
    );
  }
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(endDate)) {
    ...
    return Response.json(
      {
        success: false,
        message: "종료일 형식이 올바르지 않습니다. (YYYY-MM-DD 형식)",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (normalizedInviteCode) {
      ...
      const exists = await env.D1_DB.prepare(
        "SELECT 1 FROM challenges WHERE challenge_code = ?",
      )
    ...
    if (
      ...
      const profileRow = await env.D1_DB.prepare(
    ...
    const normalizedBetPoints =
      typeof betPoints !== "undefined" && betPoints !== null && betPoints !== ""
        ? Number(betPoints)
        : 0;
    ...
    const insertResult = await env.D1_DB.prepare(insertSql)
      .bind(...bindValues)
      .run();

    const challengeId =
      insertResult.meta.last_row_id || insertResult.lastInsertRowid;
    ...
    try {
      ...
      await env.D1_DB.prepare(
        "INSERT OR IGNORE INTO challenge_members (challenge_id, user_id, joined_at) VALUES (?, ?, datetime('now'))",
      )
        .bind(challengeId, userId)
        .run();

      const memberCountRow = await env.D1_DB.prepare(
          ...
          if (pointLogColumn) {
            await env.D1_DB.prepare(
              ...
              .bind(
                userId,
                -requiredBet,
                `challenge_bet:${challengeId}:create`,
    ...
    if (
      ...
      try {
        ...
        await env.D1_DB.prepare(
          "INSERT OR REPLACE INTO challenge_bets (challenge_id, bet_points) VALUES (?, ?)",
        )
          .bind(challengeId, Number(betPoints))
          .run();
      } catch (e) {
        console.error(
          "challenge_bets 저장 오류:",
          e instanceof Error ? e.message : String(e),
        );
      }
    }

    const createdChallenge = await env.D1_DB.prepare(
    ...
    try {
      ...
      if (
        ...
        createdChallenge &&
        createdChallenge.end_date &&
        createdChallenge.created_at
      ) {
        const createdAtStr = String(createdChallenge.created_at).replace(
          " ",
          "T",
        );
        const createdAtUTC = createdAtStr.endsWith("Z")
          ? createdAtStr
          : createdAtStr + "Z";
