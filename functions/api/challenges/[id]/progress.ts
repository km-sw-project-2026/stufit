  ...
  try {
    if (!env?.D1_DB) {
      return Response.json(
        { success: false, message: "DB not configured" },
        { status: 500 },
      );
    }
    ...
    await env.D1_DB.prepare(
      "DELETE FROM challenge_daily_progress WHERE challenge_id = ? AND user_id = ?",
    )
    ...
    await env.D1_DB.prepare(
      "DELETE FROM challenge_results WHERE challenge_id = ? AND user_id = ?",
    )
      .bind(challengeId, userId)
      .run();
