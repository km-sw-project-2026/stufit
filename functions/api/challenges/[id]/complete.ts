  ...
  try {
    ...
    if (miniGameState.hasTie && !miniGameState.allDone) {
      return Response.json({
        ...
        message:
          "공동 1등 미니게임이 진행 중입니다. 전원 완료 후 최종 종료됩니다.",
      });
    }

    await env.D1_DB.prepare(
      "UPDATE challenges SET deleted_at = CURRENT_TIMESTAMP WHERE challenge_id = ?",
    )
      .bind(challengeId)
      .run();
