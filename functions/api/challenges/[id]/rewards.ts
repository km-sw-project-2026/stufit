...
export const onRequestPost: PagesFunction<Env> = async ({
  ...
  try {
    ...
    try {
      isMember = await env.D1_DB.prepare(
    ...
    try {
      ...
      const columns = Array.isArray(pointLogInfo?.results)
        ? pointLogInfo.results
        : [];
      if (columns.some((col: any) => col.name === "point"))
        pointLogColumn = "point";
      else if (columns.some((col: any) => col.name === "points"))
        pointLogColumn = "points";
    } catch (err) {
      console.warn("[rewards] point_logs check failed:", err);
    }

    try {
      const profileInfo = await env.D1_DB.prepare(
        "PRAGMA table_info('user_profiles')",
      ).all();
      const columns = Array.isArray(profileInfo?.results)
        ? profileInfo.results
        : [];
    ...
    if (action === "giveup") {
      ...
      let memberList: any[] = [];
      try {
        const members = await env.D1_DB.prepare(
      ...
      const already = pointLogColumn
        ? await env.D1_DB.prepare(
            "SELECT 1 FROM point_logs WHERE user_id = ? AND reason = ?",
          )
            .bind(userId, reason)
            .first()
        : null;
      ...
      if (pointLogColumn) {
        try {
          ...
          console.warn(
            "POINT LOG INSERT SKIPPED:",
            pointLogErr?.message || String(pointLogErr),
          );
        }
      }
    ...
    if (memberList.length <= 1) {
      return new Response(
        JSON.stringify({
          ...
          type,
          mode,
          message: "혼자 참여한 챌린지는 보상을 받을 수 없습니다.",
        }),
        { status: 200, headers: { "Content-Type": "application/json" } },
      );
    }
    ...
    if (type !== "study") {
      try {
        const memberStatusRows = await env.D1_DB.prepare(
          `
            SELECT u.username
        ...
        const pendingMembers = Array.isArray(memberStatusRows?.results)
          ? memberStatusRows.results
          : [];
        if (pendingMembers.length > 0) {
          return new Response(
            JSON.stringify({
              ...
              mode,
              message:
                "모든 참여자가 제출을 완료해야 결과를 확인할 수 있습니다.",
            }),
            { status: 200, headers: { "Content-Type": "application/json" } },
          );
        }
    ...
    let progress;
    try {
      progress = await env.D1_DB.prepare(
        "SELECT user_id, COUNT(*) as count FROM challenge_daily_progress WHERE challenge_id = ? GROUP BY user_id",
      )
        .bind(challengeId)
        .all();
    } catch (err) {
    ...
    if (type === "study") {
      ...
      const scoreRows = Array.isArray(scoreRowsRaw?.results)
        ? scoreRowsRaw.results
        : [];
      ...
      const pendingMembers = memberList.filter(
        (m: any) => !scoreMap.has(Number(m.user_id)),
      );
      if (pendingMembers.length > 0) {
        const isSelfPending = pendingMembers.some(
          (m: any) => Number(m.user_id) === Number(userId),
        );
        return new Response(
          JSON.stringify({
            success: true,
      ...
      ranking = ranking.map((item, idx) => {
        if (idx === 0) {
          ...
          if (
            prevSubmittedScore !== null &&
            currentSubmitted !== prevSubmittedScore
          ) {
      ...
      hasTie =
        topSubmittedScore > 0 &&
        ranking.filter(
          (r) => Number(r.submittedScore || 0) === topSubmittedScore,
        ).length >= 2;
      ...
      ranking = base.map((item, idx) => {
        const reward =
          mode === "practice"
            ? { points: 0, score: 0 }
            : resolveRankReward(idx, base.length);
      ...
      hasTie =
        topRatio > 0 &&
        ranking.filter((r) => Number(r.ratio || 0) === topRatio).length >= 2;
    }
    ...
    if (hasTie && ranking.length > 0) {
      ...
      if (tiedPlayers.length >= 2) {
        try {
          ...
          const miniRowsRaw = await env.D1_DB.prepare(
            `
          ...
          const miniRows = Array.isArray(miniRowsRaw?.results)
            ? miniRowsRaw.results
            : [];
          const finishedRows = miniRows
            ...
            .sort((a: any, b: any) => {
              ...
              const wordsDiff =
                Number(b?.words_count || 0) - Number(a?.words_count || 0);
              if (wordsDiff !== 0) return wordsDiff;

              const aFinished = String(
                a?.finished_at || "9999-12-31T23:59:59.999Z",
              );
              const bFinished = String(
                b?.finished_at || "9999-12-31T23:59:59.999Z",
              );
              if (aFinished !== bFinished)
                return aFinished.localeCompare(bFinished);

              return String(a?.player_username || "").localeCompare(
                String(b?.player_username || ""),
              );
            });
          ...
          if (finishedRows.length < tiedPlayers.length) {
            return new Response(
              JSON.stringify({
                ...
                message:
                  "공동 1등 미니게임이 진행 중입니다. 종료 후 승자에게 몰빵 지급됩니다.",
                applied: [],
          ...
          if (winnerName) {
            const winnerEntry = ranking.find(
              (entry) => String(entry.name) === winnerName,
            );
            ...
            if (winnerEntry) {
              ranking = [winnerEntry, ...others].map((entry, idx) => {
                const reward =
                  mode === "practice"
                    ? { points: 0, score: 0 }
                    : resolveRankReward(idx, ranking.length);
                return {
                  ...entry,
                  rank: idx + 1,
                  points: reward.points,
    ...
    if (ranking.length > 0) {
      if (pointLogColumn) {
        try {
          const pool = await env.D1_DB.prepare(
            `SELECT SUM(CASE WHEN ${pointLogColumn} < 0 THEN -${pointLogColumn} ELSE 0 END) AS total FROM point_logs WHERE reason LIKE ?`,
          )
    ...
    for (const entry of ranking) {
      ...
      try {
        ...
        if (already) {
          console.log(
            `[rewards] skip duplicate: userId=${entry.userId} reason=${reason}`,
          );
          continue;
        }
        ...
        if (pointLogColumn) {
          try {
            ...
            console.warn(
              "[rewards] point_log insert skipped:",
              pointLogErr?.message || String(pointLogErr),
            );
          }
        }
        ...
        if (hasBet && hasScoreColumn && entry.score !== 0) {
          try {
            const scoreProfile = await env.D1_DB.prepare(
