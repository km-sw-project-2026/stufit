  ...
  try {
    ...
    if (method === "GET") {
      ...
      if (results[0].created_by_user_id !== userId) {
        return new Response(
          JSON.stringify({ ok: false, message: "Forbidden: not owner" }),
    ...
    if (method === "PATCH") {
      ...
      const { title, description, goal, end_date, max_members, is_private } =
        body;

      // Fill missing fields from DB to avoid undefined bindings
      const existing = await db
        .prepare(
          `SELECT title, description, goal, end_date, max_members, is_private FROM challenges WHERE challenge_id = ?`,
        )
        .bind(challengeId)
        .first();
      if (!existing) {
        return new Response(
          JSON.stringify({
            ok: false,
            error: "Challenge not found (during update)",
          }),
      ...
      const newTitle =
        typeof title !== "undefined" && title !== null ? title : existing.title;
      const newDescription =
        typeof description !== "undefined" && description !== null
          ? description
          : existing.description;
      const newGoal =
        typeof goal !== "undefined" && goal !== null ? goal : existing.goal;
      const newEndDate =
        typeof end_date !== "undefined" && end_date !== null
          ? end_date
          : existing.end_date;
      const newMaxMembers =
        typeof max_members !== "undefined" && max_members !== null
          ? max_members
          : existing.max_members;
      const newIsPrivate =
        typeof is_private !== "undefined" && is_private !== null
          ? is_private
          : existing.is_private;
