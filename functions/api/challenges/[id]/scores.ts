...
export const onRequest: PagesFunction<Env> = async ({
  ...
  if (!challengeId || Number.isNaN(Number(challengeId))) {
    return Response.json(
      { success: false, message: "유효하지 않은 챌린지입니다." },
      { status: 400 },
    );
  }
  ...
  if (request.method === "GET") {
    ...
    if (!userId || Number.isNaN(userId)) {
      return Response.json(
        { success: false, message: "유효한 사용자 정보가 필요합니다." },
        { status: 400 },
      );
    }
    ...
    try {
      const challenge = await env.D1_DB.prepare(
        "SELECT end_date FROM challenges WHERE challenge_id = ?",
      )
        .bind(challengeId)
        .first();
      ...
      if (!challenge) {
        return Response.json(
          { success: false, message: "챌린지를 찾을 수 없습니다." },
          { status: 404 },
        );
      }
      ...
      const row = await env.D1_DB.prepare(
        "SELECT score FROM challenge_results WHERE user_id = ? AND challenge_id = ?",
      )
      ...
      return Response.json(
        { success: false, message: "제출 상태 조회 실패" },
        { status: 500 },
      );
    }
  }
  ...
  if (Number.isNaN(score) || score < 0) {
    return Response.json(
      { success: false, message: "점수는 0 이상의 숫자여야 합니다." },
      { status: 400 },
    );
  }
  ...
  try {
        ...
        return Response.json(
          {
            ...
            expired: true,
            message:
              "점수 제출 기간이 종료되었습니다. (종료일 다음날까지만 제출 가능)",
          },
          { status: 400 },
        );
      }
    ...
    const already = await env.D1_DB.prepare(
      "SELECT 1 FROM challenge_results WHERE user_id = ? AND challenge_id = ?",
    )
      .bind(userId, challengeId)
      .first();
      ...
      return Response.json(
        {
          ...
          message: "공부 챌린지 점수는 1회만 제출할 수 있습니다.",
        },
        { status: 409 },
      );
    }
    ...
    return Response.json(
      { success: false, message: "점수 입력 실패" },
      { status: 500 },
    );
  }
};
