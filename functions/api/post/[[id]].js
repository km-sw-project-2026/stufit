export async function onRequestGet({ env, params }) {
  ...
  const result = await env.D1_DB.prepare(
    "SELECT p.*, u.username FROM posts p LEFT JOIN users u ON p.user_id = u.user_id WHERE post_id = ?",
  )
    .bind(id)
    .first();
...
export default async function onRequest(request, { env, params, userId }) {
  try {
    ...
    try {
      ...
      console.debug("[post/[[id]]] request debug", {
    ...
    if (request.method === "DELETE") {
      ...
      const commentCountRow = await env.D1_DB.prepare(
      ...
      // 2. 댓글 삭제
      await env.D1_DB.prepare("DELETE FROM comments WHERE post_id = ?")
        .bind(id)
        .run();

      // 3. 게시글 좋아요 삭제
      await env.D1_DB.prepare("DELETE FROM post_likes WHERE post_id = ?")
        .bind(id)
        .run();

      // 4. 게시글 삭제
      await env.D1_DB.prepare("DELETE FROM posts WHERE post_id = ?")
        .bind(id)
        .run();

      return Response.json({
        success: true,
        data: { postId: id, deletedCommentsCount },
        message: "게시글 삭제 완료",
      });
    }
    ...
    if (request.method === "PUT" || request.method === "PATCH") {
      ...
      await env.D1_DB.prepare(
        "UPDATE posts SET title = COALESCE(?, title), content = COALESCE(?, content), updated_at = ? WHERE post_id = ?",
      )
        .bind(title, content, now, id)
        .run();

      const updated = await env.D1_DB.prepare(
        "SELECT * FROM posts WHERE post_id = ?",
      )
        .bind(id)
        .first();
