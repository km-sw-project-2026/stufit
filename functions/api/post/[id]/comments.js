export async function onRequestGet({ env, params, userId }) {
  try {
    ...
    return Response.json(
      { success: false, message: "댓글을 불러올 수 없습니다.", error: message },
      { status: 500 },
    );
  }
}
...
export async function onRequestPost({ request, env, params, userId }) {
  try {
    ...
    const insert = await env.D1_DB.prepare(
    ...
    const created = await env.D1_DB.prepare(
    ...
    const message = err instanceof Error ? err.message : String(err);
    return Response.json(
      { success: false, message: "댓글 생성에 실패했습니다.", error: message },
      { status: 500 },
    );
  }
}
