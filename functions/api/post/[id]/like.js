export async function onRequestPost({ env, params, userId }) {
  try {
    ...
    try {
      if (count >= 200) {
        const post = await env.D1_DB.prepare(
          "SELECT user_id, popular_reward_paid FROM posts WHERE post_id = ?",
        )
          .bind(postId)
          .first();
        ...
        if (post && post.popular_reward_paid === 0) {
          // 카테고리를 'popular'로 변경
          await env.D1_DB.prepare(
            "UPDATE posts SET category = ?, popular_reward_paid = 1 WHERE post_id = ?",
          )
            .bind("popular", postId)
            .run();
          ...
          await env.D1_DB.prepare(
            "UPDATE user_profiles SET points = points + 200 WHERE user_id = ?",
          )
            .bind(authorId)
            .run();
    ...
    return Response.json({
      success: true,
      data: { liked: !exists, count, promoted },
    });
  } catch (err) {
