...
export async function onRequestGet(context: {
  ...
  try {
    ...
    // 구매한 아이템 목록 조회
    const purchasedItems = await env.D1_DB.prepare(
    ...
    const profile = await env.D1_DB.prepare(
      "SELECT profile_image_item_id, profile_border_item_id, profile_background_item_id FROM user_profiles WHERE user_id = ?",
    )
      .bind(userId)
      .first();

    const itemIds =
      purchasedItems?.results?.map((row: any) => Number(row.item_id)) || [];
...
export async function onRequestPut(context: {
  ...
  try {
    ...
    if (itemId !== null && !Number.isNaN(itemId)) {
      const hasItem = await env.D1_DB.prepare(
    ...
    if (itemType === "image") {
      updateQuery =
        "UPDATE user_profiles SET profile_image_item_id = ? WHERE user_id = ?";
    } else if (itemType === "frame") {
      updateQuery =
        "UPDATE user_profiles SET profile_border_item_id = ? WHERE user_id = ?";
    } else if (itemType === "bg") {
      updateQuery =
        "UPDATE user_profiles SET profile_background_item_id = ? WHERE user_id = ?";
    }

    await env.D1_DB.prepare(updateQuery).bind(itemId, userId).run();
