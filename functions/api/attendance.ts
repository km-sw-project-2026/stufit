...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    console.log(
      `[Attendance API GET] Fetching attendance for userId: ${userId}`,
    );
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    console.log(
      `[Attendance API] Parsed data - UserId: ${userId}, Date: ${date}`,
    );
    ...
    if (existing) {
      return new Response(
        JSON.stringify({ message: "이미 오늘 출석하셨습니다." }),
        {
          status: 409,
          headers: { "Content-Type": "application/json" },
        },
...
// interface Env {
//   // ⚠️ Cloudflare 대시보드의 바인딩 이름 'D1_DB'와 일치시켰습니다.
//   D1_DB: any;
// }
...
//   if (!env.D1_DB) {
//     return new Response(JSON.stringify({ error: "DB binding 'D1_DB' not found. Check your wrangler.json" }), {
//       status: 500,
//       headers: { "Content-Type": "application/json" }
//     });
//   }
...
//       // DB 트랜잭션 (D1_DB 사용)
//       const stmts = [
//         env.D1_DB.prepare("INSERT INTO attendance_logs (user_id, date) VALUES (?, ?)").bind(userId, date),
//         env.D1_DB.prepare("INSERT INTO point_logs (user_id, reason, point, created_at) VALUES (?, ?, ?, ?)").bind(
//           userId, `출석체크 (${date})`, rewardPoints, kstDate.toISOString()
//         ),
//         env.D1_DB.prepare("UPDATE user_profiles SET points = points + ? WHERE user_id = ?").bind(rewardPoints, userId)
//       ];
