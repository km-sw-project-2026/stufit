...
function ShopQuicklink() {
  ...
  const base = Math.max(
    56,
    Math.min(160, Math.floor((containerWidth - (VISIBLE - 1) * gap) / VISIBLE)),
  );
  const slot = base + gap;
            ...
            >
              {[-2, -1, 0, 1, 2].map((off) => {
                ...
                const leftPx = Math.round(
                  containerWidth / 2 - base / 2 + x + HORIZONTAL_SHIFT,
                );
          ...
          {images.map((_, i) => (
            <button
              ...
              style={{
                ...
                borderRadius: "50%",
                border: "none",
                background: i === centerIndex ? "#176B5F" : "#ddd",
                cursor: "pointer",
              }}
            />
          ))}
...
export default ShopQuicklink;
>>>>>>> 9aaa13e65c403a588f75d9d52e8e5d736dfe47e2
