...
function ShopQuicklink () {
    ...
    const allImages = entries.map(([k,v]) => v);
    const images = allImages.slice(0, Math.min(maxItems, allImages.length));
    ...
    useEffect(() => {
        // debug: log image urls so we can inspect network URLs in production
        if (typeof window !== 'undefined') console.log('ShopQuicklink images:', images);
    }, [images]);
                        ...
                        {images.map((src, i) => (
                            ...
                            >
                                <img
                                    src={src}
                                    alt={`item-${i}`}
                                    onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = '/img/Profile2.png'; console.error('ShopQuicklink image failed to load:', src); }}
                                    style={{ width: i === index ? 120 : 80, height: i === index ? 120 : 80, objectFit: 'contain', borderRadius: 12 }}
                                />
                            </div>
                        ))}
                    </div>
