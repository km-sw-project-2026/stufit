      ...
      try {
        ...
        const response = await fetch(`/api/user/points${query}`, {
          ...
          headers: {
            ...
            ...(Number.isInteger(normalizedUserId) && normalizedUserId > 0
              ? { "X-User-Id": String(normalizedUserId) }
              : {}),
          },
        });
        ...
        if (!Number.isNaN(dbPoints) && isMounted) {
          ...
          window.dispatchEvent(
            new CustomEvent("pointsUpdated", { detail: { points: dbPoints } }),
          );
        }
      } catch (error) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="logo">
          <Link to="/" id="logo-link" onClick={closeMobileMenu}>
        ...
        >
          <Link to="/attendance" id="attendance-link" onClick={closeMobileMenu}>
            출석체크
          </Link>
          <Link
          ...
          <Link to="/ranking" id="ranking-link" onClick={closeMobileMenu}>
            랭킹
          </Link>
          <Link to="/community" id="community-link" onClick={closeMobileMenu}>
            커뮤니티
          </Link>
          <Link to="/shop" id="shop-link" onClick={closeMobileMenu}>
            상점
          </Link>
        </div>
        ...
        <div className="auth">
          {isLoggedIn ? (
            ...
            <>
              <Link to="/login" id="login-link" onClick={closeMobileMenu}>
                로그인
              </Link>{" "}
              |{" "}
              <Link to="/signup" id="signup-link" onClick={closeMobileMenu}>
                회원가입
              </Link>
            </>
          )}
