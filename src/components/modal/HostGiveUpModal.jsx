    ...
    try {
      if (selectedOption === "leave") {
        ...
        const response = await fetch(
          `/api/challenges/${challenge.challenge_id}/leave`,
          {
            method: "DELETE",
            headers: {
              "X-Username": username,
        ...
        if (response.ok) {
          ...
          if (result.points !== undefined) {
            ...
            console.log("🔵 포인트 변경:", {
              oldPoints,
              newPoints,
              diff: newPoints - Number(oldPoints),
            });
          ...
          setAlertMessage(
            "챌린지를 나갔습니다. 방장 권한이 다른 멤버에게 이전되었습니다.",
          );
          setAlertOpen(true);
        } else {
