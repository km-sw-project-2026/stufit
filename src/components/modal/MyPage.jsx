...
function MyPage({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        // 2. 아이템 정보와 통계 정보를 병렬로 호출하여 성능 개선
        const [itemsRes, statsRes] = await Promise.all([
          fetch(`/api/user/items?userId=${userId}`, { headers: { 'X-Username': username || '' } }),
        ...
        // 3. 모든 데이터를 한 번에 업데이트 (덮어쓰기 방지)
        setUserData(prev => {
          if (!prev) return prev;
          
          const ownedCount = itemsData?.purchasedItems?.length || 0;
          const posts = statsData?.stats?.posts ?? 0;
          const comments = statsData?.stats?.comments ?? 0;
          const points = statsData?.stats?.points ?? prev.points;
  ...
  // 상점 아이템 적용 (프레임, 배경 등) 로직 - 기존 디자인 유지
  useEffect(() => {
    if (!isOpen) return;
    ...
    const applyActiveToUI = async () => {
      ...
      const applyFromActive = (active) => {
        ...
        const headerBgEl = document.querySelector('.mypage-header-bg');
        if (headerBgEl) {
          if (bgItem?.image) {
            headerBgEl.style.backgroundImage = `url(${bgItem.image})`;
            headerBgEl.style.backgroundSize = 'cover';
        ...
        const profileImgEl = document.querySelector('.profile-img img:not(.profile-frame-overlay)');
        if (profileImgEl) {
        ...
        if (frameOverlay) {
          if (frameItem?.image) {
            ...
            const widthPct = (scale * 100).toFixed(2) + '%';
            const offsetPct = (scale - 1) / 2 * 100;
            const topOffset = offsetPct + (frameItem.myPageOffsetY ?? 20);
            const leftOffset = offsetPct + (frameItem.myPageOffsetX ?? 0);
            frameOverlay.style.width = widthPct;
            frameOverlay.style.height = widthPct;
            frameOverlay.style.left = `-${leftOffset.toFixed(2)}%`;
            frameOverlay.style.top = `-${topOffset.toFixed(2)}%`;
      ...
      try {
        ...
        if (response.ok) applyFromActive(data?.activeItems || {});
      } catch (err) {
        console.error('Active items fetch error:', err);
      }
    };
    ...
    applyActiveToUI();
    window.addEventListener('activeItemsUpdated', applyActiveToUI);
    return () => window.removeEventListener('activeItemsUpdated', applyActiveToUI);
  }, [isOpen]);

  // 포인트 및 아이템 업데이트 이벤트 리스너 - 기존 유지
  useEffect(() => {
    if (!isOpen) return;
    ...
    const handlePurchasedUpdated = async () => {
      const userId = localStorage.getItem('userId');
      if (!userId) return;
      const res = await fetch(`/api/user/items?userId=${userId}`);
      const data = await res.json();
      if (res.ok) {
        setUserData(prev => prev ? { ...prev, items: `${data?.purchasedItems?.length || 0}개` } : prev);
      }
    };
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="mypage-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mypage-header">
          ...
          <div className="profile-img">
            <img src="/img/Profile2.png" alt="프로필" />
            <img src="" alt="frame" className="profile-frame-overlay" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'none', pointerEvents: 'none', zIndex: 1015 }} />
          </div>
          <div className="profile-info">
            <div className="profile-name-score">
              <h3>{userData.username}</h3>
              <div className="score-container">
                ...
                <div className="score-right-section">
                  <div className="score-bottom">
                    <span className="score-value">{userData.score}</span>
                    <button className="score-help-btn" onClick={handleTierGuideClick}>
                      <span>?</span>
                    </button>
                  </div>
                  <div className="score-progress-bar">
        ...
        <div className="mypage-activity">
          ...
          <div className="activity-section">
            <h4 onClick={handleMyItemsClick} style={{ cursor: 'pointer', color: '#4CAF50' }}>
              보유 중인 아이템 →
            </h4>
            <div className="activity-stats">
              <div className="activity-item">
