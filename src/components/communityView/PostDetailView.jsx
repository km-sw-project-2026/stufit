...
function PostDetailView({ post, onClose, onDeletePost, onEditPost, onUpdatePostState }) {
    ...
    // 좋아요 로직 수정: 200개 달성 시 포인트 지급 알림 추가
    const handleLikePost = async () => {
        if (!post?.id) return;
        try {
            const response = await fetch(`/api/post/${post.id}/like`, {
            ...
            // 서버에서 보낸 인기글 승격 및 포인트 지급 확인
            if (payload.data?.promoted) {
                setAlertModal({ 
                    show: true, 
                    message: payload.data.message || '축하합니다! 인기글 달성으로 300P가 지급되었습니다.' 
                });
            }
            ...
            const updatedPost = {
                ...
                liked: Boolean(payload.data?.liked),
                likes: Number(payload.data?.count) || 0,
                // 인기글이 되었다면 카테고리 상태도 업데이트
                category: payload.data?.promoted ? 'popular' : postState.category
            };
            setPostState(updatedPost);
            if (onUpdatePostState) onUpdatePostState(updatedPost);
        } catch (error) {
