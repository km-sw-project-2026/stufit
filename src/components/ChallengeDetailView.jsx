  ...
  const loadWeeklyData = async () => {
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        console.log(
          "[loadWeeklyData] dateStr:",
          dateStr,
          "hasSubmitted:",
          hasSubmitted,
          "userRows:",
          userRows.map((r) => r.date),
        );
    ...
    <div className="weekly-status-container">
      ...
      <div className="weekly-status-user-info">
        <div className="weekly-status-user-inner">
          ...
          />
          <span className="weekly-status-username">{username}</span>
        </div>
      </div>
      ...
      <div className="weekly-status-week-row">
        {weekData.map((day, idx) => (
          <div key={idx} className="weekly-status-day-item">
            ...
            >
              {day.hasSubmitted && (
                <span className="weekly-status-check">✓</span>
              )}
            </div>
...
function ChallengeDetailView({
  ...
  const [leaveAlertMessage, setLeaveAlertMessage] =
    useState("챌린지를 완전히 포기했습니다.");
    ...
    const loadStudySubmissionStatus = async () => {
      ...
      try {
        const response = await fetch(
          `/api/challenges/${challenge.challenge_id}/scores?userId=${userId}`,
        );
        if (!response.ok) return;
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/challenges/${challenge.challenge_id}/minigame`,
          {
  ...
  useEffect(() => {
    ...
    challenge?.challenge_id,
    challenge?.timer_hours,
    challenge?.timer_minutes,
  ]);
  ...
  const getTotalDays = () => {
    ...
    if (
      typeof challenge?.duration !== "undefined" &&
  ...
  const giveupHandler = () => {
    ...
    const isHost =
      currentUser && challenge?.created_by_user_id === currentUser.user_id;
  ...
  const handleSubmitProgress = async () => {
    ...
    if (!isChallengeStarted) {
      ...
      alert(
        `인원을 모두 모아야 합니다. (현재 ${members.length}명 / 필요 ${challenge?.max_members}명)`,
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        "[handleSubmitProgress] API 호출 시작, todayInSeoul:",
        todayInSeoul,
      );
      ...
      console.log(
        "[handleSubmitProgress] 제출 성공! setSubmittedToday(true) 호출",
      );
      ...
      console.log(
        "[handleSubmitProgress] 제출 중 상태 해제, submittedToday:",
        submittedToday,
      );
    }
  };
  ...
  const handleLeaveSuccess = (actionType = "leave") => {
    ...
    try {
      ...
      console.warn("leave local clear failed", e);
    }
    // 자세히보기도 닫기
    setTimeout(() => {
      if (onClose) onClose();
  ...
  const handleComplete = () => {
    console.log("[handleComplete] click", {
      remainingDays,
  ...
  const finalizeChallenge = async (overrideStudyScore = null) => {
    ...
    const buildAuthHeaders = (base = {}) => {
      ...
      if (!Number.isNaN(myUserId) && myUserId > 0)
        headers["X-User-Id"] = String(myUserId);
      return headers;
    };
    ...
    const syncPointsFromServer = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/user/stats?userId=${myUserId}&t=${Date.now()}`,
          {
            headers,
          },
        );
    ...
    try {
      ...
      if (
        getChallengeType() === "study" &&
        resolvedStudyScore !== null &&
        !Number.isNaN(Number(resolvedStudyScore))
      ) {
        rewardsBody.score = Number(resolvedStudyScore);
      }
      const rewardsResponse = await fetch(
        `/api/challenges/${challenge.challenge_id}/rewards`,
        {
          method: "POST",
          headers,
      ...
      if (rewardsResponse.ok) {
        ...
        if (payload?.pendingMinigame) {
          setChallengeOverNotice(
            payload?.message || "공동 1등 미니게임 결과를 기다리는 중입니다.",
          );
          if (Array.isArray(payload?.ranking)) {
            setRankingData(payload.ranking);
          }
          if (
            Array.isArray(payload?.tiedPlayers) &&
            payload.tiedPlayers.length >= 2
          ) {
            setHasTie(true);
        ...
        if (payload?.pendingSubmission) {
          ...
          const notice =
            payload?.message || `아직 ${pendingCount}명이 제출하지 않았습니다.`;
          setChallengeOverNotice(notice);

          if (getChallengeType() === "study") {
            const pendingMembers = Array.isArray(payload?.pendingMembers)
              ? payload.pendingMembers
              : [];
        ...
        if (Array.isArray(payload?.ranking)) {
          ...
          if (payload.hasTie !== undefined) {
            ...
            if (detectedTie) {
              const topScore = Number(
                payload.ranking[0]?.submittedScore ??
                  payload.ranking[0]?.score ??
                  0,
              );
              tiedList = payload.ranking
                .filter(
                  (r) =>
                    Number(r?.submittedScore ?? r?.score ?? 0) === topScore &&
                    topScore > 0,
                )
                .map((r) => r.name);
            }
          ...
          if (!detectedTie) {
            try {
              ...
              await fetch(
                `/api/challenges/${challenge.challenge_id}/complete`,
                {
                  method: "PATCH",
            ...
            console.log(
              "[finalizeChallenge] tie detected - skip challenge complete until minigame finished",
            );
          }
        ...
        const errText =
          errPayload?.message ||
          errPayload?.error ||
          `보상 정산에 실패했습니다. (status ${rewardsResponse.status})`;
        console.warn(
          "[finalizeChallenge] rewards failed:",
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("challengeCompleted", { detail: { delta: 1 } }),
      );
  ...
  const handleSubmitStudyScore = async (score) => {
    ...
    try {
      const response = await fetch(
        `/api/challenges/${challenge.challenge_id}/scores`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        ...
        if (payload?.alreadySubmitted) {
          ...
          setChallengeOverNotice(
            payload?.message ||
              "이미 점수를 제출했습니다. 다른 참여자의 제출을 기다려 주세요.",
          );
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      if (res.status === 404) {
        console.warn(
          "[fetchMembers] challenge not found (404):",
          challenge.challenge_id,
        );
        setMembers([]);
        return;
      ...
      if (payload?.data) {
        console.log(
          "[fetchMembers] challenge max_members:",
          payload.data.max_members,
        );
      ...
      try {
        ...
        if (e?.detail?.challengeId === challenge?.challenge_id) {
          if (Array.isArray(e.detail.members)) {
            console.debug(
              "challenge-joined: updating members from event",
              e.detail.members,
            );
            ...
            try {
              ...
              if (
                username &&
                e.detail.members.some((m) => m.username === username)
              ) {
                setSubmittedToday(false);
                setRefreshKey((prev) => prev + 1);
              }
            } catch (err) {
              console.warn("clear submitted on joined failed", err);
            }
          } else {
            console.debug(
              "challenge-joined: members not in event, reloading from server",
            );
        ...
        console.error("challenge-joined handler error", err);
      }
    };
      ...
      try {
        ...
        if (e?.detail?.challengeId === challenge?.challenge_id) {
          if (e.detail.challenge) {
            console.debug(
              "challenge-updated: updating challenge",
              e.detail.challenge,
            );
          ...
          if (Array.isArray(e.detail.members)) {
            console.debug(
              "challenge-updated: updating members",
              e.detail.members,
            );
            setMembers(e.detail.members);
            ...
            try {
              const username = localStorage.getItem("username");
              if (username) {
                const myRec = e.detail.members.find(
              ...
              console.warn("sync submitted on update failed", err);
            }
          }
        }
      } catch (err) {
        console.error("challenge-updated handler error", err);
      }
    };
  ...
  return (
    <>
      <div id="challenge-detail-view" className={isPage ? "" : "modal"}>
        <div className={`detail-view-container ${isPage ? "is-page" : ""}`}>
          ...
          <div className="detail-main">
            ...
            <div className="detail-card">
              ...
              <button
                ...
                disabled={
                  submittedToday || submitLoading || !isChallengeStarted
                }
                style={{ marginTop: "-4px" }}
              >
                {!isChallengeStarted
            ...
            <div className="detail-actions" style={{ marginTop: "6px" }}>
              <button className="btn-giveup" onClick={giveupHandler}>
                give up
              </button>
      ...
      <ChallengeOverModal
        ...
        onStartMiniGame={async (players) => {
          ...
          if (!res.ok) {
            ...
            console.warn(
              "[onStartMiniGame] non-host or already exists:",
              err?.message,
            );
          }
        }}
