...
function UserProfilePreviewModal({
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        setActiveItems(
          itemsPayload?.activeItems || { image: null, frame: null, bg: null },
        );
        ...
        const completedChallenges =
          Number(statsPayload?.stats?.completedChallenges) || 0;
        const itemCount = Array.isArray(itemsPayload?.purchasedItems)
          ? itemsPayload.purchasedItems.length
          : 0;
        ...
        const rankIndex = rankedUsers.findIndex(
          (user) => Number(user.userId) === Number(resolvedUserId),
        );
        const currentRank = rankIndex >= 0 ? `${rankIndex + 1}위` : "-";

        setUserData({
          username:
            username || rankedUsers[rankIndex]?.username || "알 수 없는 사용자",
          score,
          points,
          posts,
          comments,
  ...
  const bgItem = useMemo(
    () => shopItems.find((item) => item.id === Number(activeItems?.bg)),
    [activeItems],
  );
  const frameItem = useMemo(
    () => shopItems.find((item) => item.id === Number(activeItems?.frame)),
    [activeItems],
  );
  const imageItem = useMemo(
    () => shopItems.find((item) => item.id === Number(activeItems?.image)),
    [activeItems],
  );
  ...
  const formatPoints = (value) =>
    `${(Number(value) || 0).toLocaleString("ko-KR")} P`;
  ...
  const modalPositionStyle = useMemo(() => {
    ...
    const left = Math.max(
      16,
      Math.min(leftBase, viewportWidth - modalWidth - 16),
    );
    const clickCenterY = Math.round(
      anchorPosition.top + (anchorPosition.height || 0) / 2,
    );
    const top = Math.max(
      16,
      Math.min(
        clickCenterY - Math.round(modalHeight / 2),
        viewportHeight - modalHeight - 16,
      ),
    );
    return {
      left: `${left}px`,
            ...
            <div className="activity-stats">
              <div className="activity-item">
                <span className="activity-label">
                  글쓰기 {userData?.posts ?? 0}개
                </span>
              </div>
              <div className="activity-item">
                <span className="activity-label">
                  댓글 {userData?.comments ?? 0}개
                </span>
              </div>
            </div>
          </div>
