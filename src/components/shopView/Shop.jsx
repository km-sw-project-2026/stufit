...
function Shop() {
  ...
  const isWishlisted = (scope, id) =>
    Boolean(wishlistItemsByKey[buildWishlistKey(scope, id)]);
  const isPurchased = (itemKey) => Boolean(purchasedItemsByKey[itemKey]);
  const isPurchasedByScope = (scope, id) =>
    isPurchased(buildWishlistKey(scope, id));
    ...
    if (!resolvedUserId && currentUsername) {
      try {
        const response = await fetch(
          `/api/user/resolve?username=${encodeURIComponent(currentUsername)}`,
        );
    ...
    try {
      ...
      if (!Number.isNaN(nextPoints)) {
        ...
        window.dispatchEvent(
          new CustomEvent("pointsUpdated", { detail: { points: nextPoints } }),
        );
      }
      ...
      const wishlistKey =
        item?._wishlistKey ?? buildWishlistKey(scope, item?.id);
  ...
  useEffect(() => {
    const resolveUserId = async () => {
      ...
      try {
        const response = await fetch(
          `/api/user/resolve?username=${encodeURIComponent(username)}`,
          {
      ...
      <div className="shop-main">
        <div className="shop-count-row">
          <div className="shop-count">
            <span className="shop-count-number">
              {itemCounts[activeCategory] ?? 0}
            </span>{" "}
            항목이 있어요
          </div>
