...
function Attendance() {
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      ...
      try {
        const response = await fetch(`/api/attendance?userId=${userId}&t=${new Date().getTime()}`);
        if (response.ok) {
          let data = await response.json(); // const를 let으로 변경 (수정 가능하게)

          // 🚀 [TEST] 가짜 데이터 주입 코드 시작
          // 28번 유저로 로그인했을 때만 수요일(11일), 목요일(12일) 도장을 강제로 만듭니다.
          if (userId === "28") {
            data.logs = [
              { date: "2026-02-11" }, // 수요일
              { date: "2026-02-12" }  // 목요일
  ...
  // ... (이하 handleCardClick 및 return 부분은 기존과 동일하므로 생략)
 const showAlert = (msg) => {
    setAlertMessage(msg);
    setIsAlertOpen(true);
  };
