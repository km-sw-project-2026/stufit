...
function Attendance() {
  ...
  const handleCardClick = async (index) => {
    ...
    try {
      const response = await fetch('/api/attendance', {
        ...
        body: JSON.stringify({ userId: Number(userId), date: todayDateString })
      });
      ...
      if (response.ok) {
        ...
        setCheckedDays(newCheckedDays);
        setLastCheckDate(todayDateString);
        localStorage.setItem('stufit_attendance', JSON.stringify({ days: newCheckedDays, date: todayDateString }));
  ...
  return (
    <div className="attendance-section">
      ...
      {isAlertOpen && (
        <CustomAlertModal message={alertMessage} onClose={() => setIsAlertOpen(false)} />
      )}
    </div>
  );
}
